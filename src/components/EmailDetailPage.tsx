...
const EmailDetailPage: React.FC = () => {
    ...
    const [email, setEmail] = useState<DealRecapEmail | null>(null);
    ...
    const emailId = emailIdParam ? parseInt(emailIdParam, 10) : null;
    ...
    useEffect(() => {
        ...
        const loadEmail = async () => {
            try {
                ...
                const emailDataModule = await import('../data/deal_recap_email.json');
                const emailObjects = emailDataModule.default as Array<{id: number, email: string}>;
                
                const emailObj = emailObjects.find(obj => obj.id === emailId);
                if (!emailObj) {
                ...
                const subject = subjectMatch ? subjectMatch[1] : `Email Chain ${emailId}`;
                ...
                const emailData: DealRecapEmail = {
                    id: emailObj.id,
                    subject,
                    content,
                ...
                // Check if this email has already been extracted
                await checkForExistingDeal(emailId);
    ...
    const checkForExistingDeal = async (emailId: number) => {
        try {
            const sessionStorage = SessionStorageService.getInstance();
            await sessionStorage.initialize();
            
            const deals = sessionStorage.getDeals();
            const existingDeal = deals.find(deal => deal.email_id === emailId);
            ...
            <div className="email-detail-content">
                ...
                {!relatedDeal && (
                    <div className="email-actions">
                        <button 
                            className="btn btn-extract-deal btn-large" 
